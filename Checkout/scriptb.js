...
function main(link,email,variant,firstname,lastname,address,city,state,country,phone,zip,step=3){
  ...
  createcheckout(link,body,f,step)
}
  
async function createcheckout(link,body,time,step){
    ...
    await request(options,function(error,response,body){
      if (response.headers['location'] == undefined){
        ...
        if (response.headers['location'].includes("checkouts")){
          getshiprates(link,(JSON.parse(body)['checkout']['token']),cj,auth,shopid,time)
...
async function getshiprates(link,token,jar,auth,id,time){
  //inputs for request
  options = {
    method: "GET",
  ...
  async function req(){
    await request(options,function(error,response,body){
      ...
      if (response.statusCode == 202){
        ...
        submitshipping(link,token,jar,(JSON.parse(body)['shipping_rates'][0]['id']),auth,time)
      }
    })
  }
...
async function submitshipping(link,token,jar,sm,auth,time){
  //headers for request
  const head = { 
        "User-Agent":"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36",
        'Content-Type': 'application/json',
  ...
  await request(options2,function(error,response,body){
    if ("checkout" in JSON.parse(body)){
      ...
      return(JSON.parse(body)['checkout']['web_url'])
    }else{
